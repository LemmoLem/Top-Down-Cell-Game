...
void Sprite::Init(float colour[3], const char* filename, float repeatInS, float repeatInT)
{
	...
	tex[0] = 0.0f;	 tex[1] = repeatInT * 1.0;
	tex[2] = 0.0f;	 tex[3] = 0.0;
	tex[4] = repeatInS * 1.0f;	 tex[5] = 0.0;

	tex[6] = 0.0f;	 tex[7] = repeatInT * 1.0;
	tex[8] = repeatInS * 1.0f;	 tex[9] = repeatInT * 1.0;
	tex[10] = repeatInS * 1.0f;	 tex[11] = 0.0;
...
void Sprite::SetYMoveAmount(float amount) {
	yMoveAmount = amount;
}
void Sprite::SetXMoveAmount(float amount) {
	xMoveAmount = amount;

}

float Sprite::GetYMoveAmount() {
	return yMoveAmount;
}
float Sprite::GetXMoveAmount() {

	return xMoveAmount;
}

void Sprite::SetMaxSpeed(float amount) {
	maxSpeed = amount;
}
